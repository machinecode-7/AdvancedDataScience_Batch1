...
delete from prime_ministers where country = 'United Kingdom';
...
-- inner join is all about common  column values between two tables, no nulls.
select count(*)
from 
prime_minister_terms pm1
inner join prime_ministers pm2
...
from 
prime_minister_terms pm1
right join prime_ministers pm2
on pm1.pm = pm2.prime_minister;

-- case 
select * from prime_minister_terms;

create table pm_modified as 
select pm, pm_start,
case 
when pm_start > 2014 then 'Young'
when pm_start <2000 then 'Old'
else 'Medium'
end as Maturity
from prime_minister_terms;

-- SubQueries
select count(*) from (select * from prime_minister_terms where pm_start >= 2015) subquery;


-- Write
-- update, delete, alter
-- Read
-- select, when, where, limit

select * from prime_minister_terms where pm_start >= 2015;

select * from states;

select * from states where indep_year <= 1950;

select * from (select * from states where indep_year <= 1950) x where continent='Asia';

select * from states where indep_year <= 1950 and continent = 'Asia';